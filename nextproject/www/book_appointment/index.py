...
@frappe.whitelist(allow_guest=True)
def create_appointment(date, time, tz, contact,referral_code=None):
	print(referral_code,'-----*******************---')
	format_string = "%Y-%m-%d %H:%M:%S"
	...
	scheduled_time = scheduled_time.replace(tzinfo=None)
	# Create a appointment document from the form 
	...
	if referral_code:
		appointment.custom_referral_code = referral_code
		sales_partner = frappe.db.get_value("Sales Partner", {"referral_code": referral_code}, 'name')
		if sales_partner:
			appointment.custom_sales_partner = sales_partner
		else:
			frappe.throw("Refreeal Code Not Found")
