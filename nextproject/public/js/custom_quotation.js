    ...
    refresh:function(frm){
        ...
        if(!frm.is_new() && frm.doc.quotation_to === "Customer"){
            frm.add_custom_button('New Email', () => {
                frappe.call({
                    method:'nextproject.custom_quotation.get_customer_contacts',
                    args:{
            ...
            dg = frappe.views.custom_quotation_email = class {
                constructor(opts) {
                    $.extend(this, opts);
                    if (!this.doc) {
        ...
        else if(!frm.is_new() && frm.doc.quotation_to === "Lead"){
            frm.add_custom_button('New Email', () => {
                frappe.call({
                    method:'nextproject.custom_quotation.get_lead_contacts',
                    args:{
                        name:frm.doc.party_name,
            ...
            dg = frappe.views.custom_quotation_email = class {
                ...
                setup_multiselect_queries() {
                    ['recipients','bcc','cc'].forEach(field => {
                        this.dialog.fields_dict[field].get_data = () => {
                            const data = this.dialog.fields_dict[field].get_value();
                            const txt = data.match(/[^,\s*]*$/)[0] || '';
                            ...
                            frappe.call({
                                ...
                                callback: (r) => {
                                    ccs = r.message
                                    // console.log("ttype",this.frm.meta.name)
                                    this.dialog.fields_dict[field].set_data(ccs);
                                }
                            });
                        };
                    });
